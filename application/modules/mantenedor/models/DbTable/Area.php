...
class Mantenedor_Model_DbTable_Area extends Zend_Db_Table_Abstract
{
    ...
    public function actualizar($datos,$id_registro){
        if(is_array($datos) && is_numeric($id_registro)){
            if($this->update($datos,'id_area = '.$id_registro)) return true; else return false;
        }else return false;
    }
    public function eliminar($id_registro){
        if(is_numeric($id_registro)){
            if($this->delete('id_area = '.$id_registro)) return true; else return false;
        }else return false;
    }
}
